...
//!
//! ## C# оригинал
//! `Proteus.AI/Services/StrategyEvolver.cs`
...
const SPLIT_POS_CANDIDATES: &[&str] = &[
    "1", "2", "3", "7", "10", "1+s", "2+s", "3+s", "host", "midsld", "sniext",
];
...
const PSIPHON_COUNTRIES: &[&str] = &[
    "AT", "AU", "BE", "BG", "CA", "CH", "CZ", "DE", "DK", "EE", "ES", "FI", "FR", "GB", "HR", "HU",
    "IE", "IN", "IT", "JP", "LV", "NL", "NO", "PL", "PT", "RO", "RS", "SE", "SG", "SK", "US",
];
...
impl StrategyEvolver {
    ...
    pub fn evolve(
        ...
        let mut scored: Vec<(&StrategyGenome, f64, usize)> = pool
            .iter()
            .map(|g| {
                let filtered: Vec<&(Uuid, i32)> =
                    outcomes.iter().filter(|(id, _)| *id == g.id).collect();
        ...
        let parents: Vec<&StrategyGenome> =
            scored[..parent_count].iter().map(|(g, _, _)| *g).collect();
        ...
        let mut child = child.ok_or_else(|| {
            AiError::Evolution("failed to create valid child after 25 attempts".into())
        })?;
    ...
    fn crossover(&mut self, a: &StrategyGenome, b: &StrategyGenome) -> StrategyGenome {
        StrategyGenome {
            ...
            desync_any_protocol: self
                .rng_pick_opt_ref(&a.desync_any_protocol, &b.desync_any_protocol),
    ...
    fn mutate_zapret(&mut self, g: &mut StrategyGenome, roll: i32) {
        match roll {
            0 => {
                g.split_pos_semantic = Some(
                    SEMANTIC_MARKERS[self.rng.gen_range(0..SEMANTIC_MARKERS.len())].to_string(),
                );
                g.split_pos = None;
            }
            1 => {
            ...
            3 => {
                g.fake_tls_mod =
                    Some(FAKE_TLS_MODS[self.rng.gen_range(0..FAKE_TLS_MODS.len())].to_string());
            }
            4 => {
                g.auto_ttl = !g.auto_ttl;
            }
            5 => {
                g.desync_fooling = Some(
                    FOOLING_CANDIDATES[self.rng.gen_range(0..FOOLING_CANDIDATES.len())].to_string(),
                );
            }
            6 => {
                g.desync_any_protocol = Some(
                    ANY_PROTOCOL_CANDIDATES[self.rng.gen_range(0..ANY_PROTOCOL_CANDIDATES.len())]
                        .to_string(),
                );
            }
            7 => {
                g.fake_resend = Some(
                    FAKE_RESEND_CANDIDATES[self.rng.gen_range(0..FAKE_RESEND_CANDIDATES.len())]
                        .to_string(),
                );
            }
            8 => {
                g.repeat_count = Some(
                    REPEAT_COUNT_CANDIDATES[self.rng.gen_range(0..REPEAT_COUNT_CANDIDATES.len())],
                );
            }
            9 => {
                g.disorder_pos = Some(
                    DISORDER_POS_CANDIDATES[self.rng.gen_range(0..DISORDER_POS_CANDIDATES.len())]
                        .to_string(),
                );
            }
            10 => {
                g.fake_pos = Some(
                    FAKE_POS_CANDIDATES[self.rng.gen_range(0..FAKE_POS_CANDIDATES.len())]
                        .to_string(),
                );
            }
            11 => {
            ...
            12 => {
                g.oob_pos = Some(
                    OOB_POS_CANDIDATES[self.rng.gen_range(0..OOB_POS_CANDIDATES.len())].to_string(),
                );
            }
            13 => {
                g.disoob_pos = Some(
                    DISOOB_POS_CANDIDATES[self.rng.gen_range(0..DISOOB_POS_CANDIDATES.len())]
                        .to_string(),
                );
            }
            14 => {
                g.tlsrec_pos = Some(
                    TLSREC_POS_CANDIDATES[self.rng.gen_range(0..TLSREC_POS_CANDIDATES.len())]
                        .to_string(),
                );
            }
            _ => {}
        ...
        match roll {
            ...
            4 => {
                ...
                if g.psiphon_enabled {
                    g.psiphon_country = Some(
                        PSIPHON_COUNTRIES[self.rng.gen_range(0..PSIPHON_COUNTRIES.len())]
                            .to_string(),
                    );
                }
            }
    ...
    fn mutate_byedpi(&mut self, g: &mut StrategyGenome, roll: i32) {
        match roll {
            0 => {
                g.split_pos = None;
                g.split_pos_semantic = Some(
                    SPLIT_POS_CANDIDATES[self.rng.gen_range(0..SPLIT_POS_CANDIDATES.len())]
                        .to_string(),
                );
            }
            1 => {
            ...
            8 => {
                g.auto = Some(if self.rng.gen_bool(0.5) {
                    "torst".into()
                } else {
                    "ssl_err".into()
                });
            ...
            9 => {
                g.mod_http = Some(
                    MOD_HTTP_CANDIDATES[self.rng.gen_range(0..MOD_HTTP_CANDIDATES.len())]
                        .to_string(),
                );
            }
            10 => {
    ...
    pub fn gc_evolved(
        ...
        let (evolved, others): (Vec<StrategyGenome>, Vec<StrategyGenome>) = pool
            .drain(..)
            .partition(|g| g.origin == StrategyOrigin::Evolved);
        ...
        let _protected: Vec<Uuid> = ranked
            .iter()
            .take(elitism_count)
            .map(|(g, _)| g.id)
            .collect();
...
mod tests {
    ...
    fn test_crossover_mixes_params() {
        ...
        assert!(
            child.engine_type == DpiEngineType::Zapret
                || child.engine_type == DpiEngineType::ByeDpi
        );
    ...
    fn test_mutate_zapret_changes_param() {
        ...
        let _original_desync = g.desync_mode.clone();
