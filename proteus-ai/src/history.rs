...
//!
//! ## C# оригинал
//! `Proteus.AI/Services/AiHistoryStore.cs`
...
impl AiHistoryStore {
    ...
    pub fn append(&mut self, record: HistoryRecord) -> Result<(), AiError> {
        ...
        if !self.file_path.as_os_str().is_empty() {
            ...
            writeln!(file, "{line}").map_err(|e| AiError::History(format!("cannot write: {e}")))?;
        }
...
mod tests {
    ...
    fn test_for_genome_on_network() {
        ...
        store
            .append(HistoryRecord::new(g1.id, "net-a", 80, 50.0))
            .unwrap();
        store
            .append(HistoryRecord::new(g1.id, "net-b", 30, 200.0))
            .unwrap();
        store
            .append(HistoryRecord::new(g2.id, "net-a", 95, 100.0))
            .unwrap();
