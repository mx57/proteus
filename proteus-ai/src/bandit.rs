...
//!
//! ## C# оригинал
//! `Proteus.AI/Services/BanditSelector.cs`
...
impl BanditSelector {
    ...
    pub fn record_trial(
        ...
        let local = self
            ...
            .entry(network_hash.to_string())
            .or_default();
        let arm = local.entry(genome_id.to_string()).or_default();
        arm.update(success, latency_ms);

        let global_arm = self.global_arms.entry(genome_id.to_string()).or_default();
