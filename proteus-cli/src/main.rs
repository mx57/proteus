...
use proteus_ai::FingerprintProvider;
...
#[command(
    name = "proteus",
    version,
    about = "Proteus — самообучающаяся система обхода DPI"
)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
fn cmd_fingerprint() {
    ...
    println!();
    match proteus_ai::fingerprint::BasicFingerprintProvider::new(
        "WLAN",
        "Wi-Fi",
        "192.168.1.1",
        vec!["8.8.8.8".into()],
    ...
    .current_fingerprint()
    {
        ...
        None => println!("  ❌ Error: Could not get fingerprint"),
    }
}
