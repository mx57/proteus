...
impl SelfUpdater {
    pub fn new(repo: &str, current_version: &str, channel: UpdateChannel) -> Self {
        ...
        Self {
            ...
            repo_name: parts.get(1).copied().unwrap_or("Proteus_AI").to_string(),
...
mod tests {
    ...
    async fn test_updater_creation() {
        let updater = SelfUpdater::new("mx57/Proteus_AI", "1.0.0", UpdateChannel::Stable);
        assert_eq!(updater.current_version(), "1.0.0");
        assert_eq!(updater.channel(), UpdateChannel::Stable);
        assert_eq!(updater.repo(), "mx57/Proteus_AI");
    }
