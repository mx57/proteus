...
impl AiOrchestratorService {
    ...
    pub async fn begin_fingerprinting(&self) -> Result<(), AiError> {
        ...
        if *state != OrchestratorState::Idle && *state != OrchestratorState::Verifying && *state != OrchestratorState::Evolving {
            return Err(AiError::Orchestrator(format!("cannot start fingerprinting from state {:?}", *state)));
        }
    ...
    pub fn verify_failure(&mut self, evolve: bool) -> Result<(), AiError> {
        ...
        if evolve && self.config.auto_evolve {
            ...
            self.state = OrchestratorState::Selecting;
    pub async fn complete_fingerprinting(&self, fingerprint: NetworkFingerprint) -> Result<(), AiError> {
        let mut state = self.state.lock().await;
        if *state != OrchestratorState::Fingerprinting {
            return Err(AiError::Orchestrator(format!("cannot complete fingerprinting from state {:?}", *state)));
        }
    ...
    pub async fn begin_selection(&self) -> Result<(), AiError> {
        let mut state = self.state.lock().await;
        if *state != OrchestratorState::Fingerprinting && *state != OrchestratorState::Idle && *state != OrchestratorState::Verifying && *state != OrchestratorState::Evolving {
             return Err(AiError::Orchestrator(format!("cannot start selection from state {:?}", *state)));
        }
        *state = OrchestratorState::Selecting;
        Ok(())
    }
    ...
    pub async fn complete_selection(&self, strategy: StrategyGenome) -> Result<(), AiError> {
        ...
        if *state != OrchestratorState::Selecting {
            return Err(AiError::Orchestrator(format!("cannot complete selection from state {:?}", *state)));
        }
    ...
    pub async fn begin_execution(&self) -> Result<(), AiError> {
        ...
        if *state != OrchestratorState::Selecting && *state != OrchestratorState::Idle {
            return Err(AiError::Orchestrator(format!("cannot start execution from state {:?}", *state)));
        }
        *state = OrchestratorState::Executing;
        Ok(())
    }
    ...
    pub async fn complete_execution(&self) -> Result<(), AiError> {
        ...
        if *state != OrchestratorState::Executing {
            return Err(AiError::Orchestrator(format!("cannot complete execution from state {:?}", *state)));
        }
    ...
    pub async fn complete_verification(&self, result: VerificationResult) -> Result<(), AiError> {
        ...
        if *state != OrchestratorState::Verifying {
             return Err(AiError::Orchestrator(format!("cannot complete verification from state {:?}", *state)));
        }
    ...
    pub async fn complete_evolution(&self) -> Result<(), AiError> {
        ...
        if *state != OrchestratorState::Evolving {
             return Err(AiError::Orchestrator(format!("cannot complete evolution from state {:?}", *state)));
        }
...
mod tests {
    ...
    async fn test_orchestrator_state_transitions() {
        ...
        assert_eq!(orchestrator.state().await, OrchestratorState::Fingerprinting);
        ...
        let fingerprint = NetworkFingerprint {
            ...
            dns_servers: vec![],
            local_subnet: "1.2.3.0/24".to_string(), captured_at: chrono::Utc::now(),
        };
        orchestrator.complete_fingerprinting(fingerprint).await.unwrap();
    ...
    async fn test_orchestrator_failure_triggers_evolution() {
        ...
        orchestrator.complete_verification(fail_result.clone()).await.unwrap();
        assert_eq!(orchestrator.state().await, OrchestratorState::Selecting);
        ...
        orchestrator.complete_verification(fail_result).await.unwrap();
