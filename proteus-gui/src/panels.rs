    ...
    egui::ScrollArea::vertical().show(ui, |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            if ui.button("▶ Evolve").clicked() {
                ...
                state.add_log(
                    "info",
                    format!("Evolution triggered: gen {}", state.evolver_generation),
                );
            }
        });
    });
...
pub fn settings_tab(ui: &mut egui::Ui, state: &mut AppState) {
    egui::ScrollArea::vertical().show(ui, |ui| {
        ...
        ui.add(
            egui::Slider::new(&mut state.check_interval_secs, 5..=300).text("Check Interval (s)"),
        );
        ui.add(
            egui::Slider::new(&mut state.evolution_interval_mins, 1..=1440)
                .text("Evolution Interval (min)"),
        );
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            for entry in state.logs.iter().rev() {
                ...
                ui.colored_label(
                    color,
                    format!("[{}] [{}] {}", time, entry.level, entry.message),
                );
            }
        });
}
